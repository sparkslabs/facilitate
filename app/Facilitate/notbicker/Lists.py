...
#
def get_itemtypes(dirlist = ["lists","items","comments"]):               # Prolly should get moved to Record.py
    itemtype_list=[]
    id = 0 
    for dir in dirlist:
...
class RelationRender(object):
    record_listview_template = 'templates/Lists.View.tmpl'
    record_editget_template = 'templates/List.Edit.tmpl' # Used ?
    record_editget_items_template = 'templates/Lists.Items.Edit.tmpl'
    record_view_template = 'templates/List.View.tmpl'
...
def RenderedRelationEntryForm(relationid, LeftRelationName, RightRelationName, leftkey,rightkey, LeftDB, RightDB, **extra_args):
    ...
    empty_data_entry = Template ( file = "templates/Lists.Items.Edit.tmpl",
                                 searchList = [
...
def page_render_html(json, **argd):
    ...
    if action == "create_new":
        ...
        relations = read_database()
        rendered_relations   = R.rendered_record_list(relations)
        if argd.get("form.left_dbid")=="lists": # now we handle building the tree
            for relation in relations:                  # check lhs existence 
                if argd.get("form.left_itemid") == relation["left_itemid"] and relation["left_dbid"]=="lists":
                    duplicate_content="<B> Record Not Saved </B>. The relation you requested already exists"
                    return R.render_page(content=rendered_relations,dataentry=duplicate_content)
            parent_record = get_item_record("lists", argd.get("form.left_itemid"))
...
#            argd["form.root_id"]=-1
    ...
    if action == "delete_relation":
        ...
        delete_action = "<a href='/cgi-bin/app/lists?formtype=confirm_delete&relationid=%s'>%s</a>" % (relation["relationid"], "Delete this item")
        cancel_action = "<a href='/cgi-bin/app/lists?formtype=view_relation&relationid=%s'>%s</a>" % (relation["relationid"], "Cancel deletion")
