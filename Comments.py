...
def update_comment(stem="form", **argd):
    the_comment = {
        "commentid" : argd.get(stem + ".commentid",""),
        "comment" : argd.get(stem + ".comment",""),
...
def page_render_html(json, **argd):
    ...
    if action == "overview":
        ...
        return R.render_page(content=rendered_comments) #) str(
    ...
    if action == "create_new":
        ...
        os.sys.stderr.write(argd.get("form.comment"))
        comments = read_database()
        rendered_comments     = R.rendered_record_list(comments)
    ...
    if action == "delete_comment":
        ...
        comment_rendered = "<h3> Are you sure you wish to delete this comment?</h3><ul>" + str(comment_rendered)

        delete_action = "<a href='/cgi-bin/app/comments?formtype=confirm_delete_comment&commentid=%s'>%s</a>" % (comment["commentid"], "Delete this comment")
        cancel_action = "<a href='/cgi-bin/app/comments?formtype=view_comment&commentid=%s'>%s</a>" % (comment["commentid"], "Cancel deletion")
    ...
    if action == "confirm_delete_comment":
        # Show the database & a few options
        comment = get_comment(argd["commentid"])
        delete_comment(argd["commentid"])

        comments = read_database()
        rendered_comments          = R.rendered_record_list(comments)
        person_rendered = R.rendered_record(comment)

        return R.render_page(content=rendered_comments, dataentry="<h1> %s Deleted </h1>" % comment["commentid"])
