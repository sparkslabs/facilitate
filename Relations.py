...
class RelationRender(object):
    ...
    def rendered_itemtypes(self, relationid, itemtypes):
         rendered_itemtypes = Template( file = self.itemtypes_view_template,
				      searchList = [self.environ,
                                                    {"relationid":relationid}, {"itemtypes":itemtypes}])
...
def page_render_html(json, **argd):
    ...
    if action == "edit_relation":
        ...
        empty_data_entry = R.rendered_itemtypes(argd["relationid"],leftitemtypes)
        configured_form = R.render_configured_form(empty_data_entry, nextstep="update_items", submitlabel="Update Item", )
        return R.render_page(extra=rendered_relations,content = rendered_relation_pair, dataentry=configured_form)
    ...
    if action == "delete_relation":
        ...
        delete_action = "<a href='/cgi-bin/app/relations?formtype=confirm_delete&relationid=%s'>%s</a>" % (relation["relationid"], "Delete this item")
        cancel_action = "<a href='/cgi-bin/app/relations?formtype=view_relation&relationid=%s'>%s</a>" % (relation["relationid"], "Cancel deletion")
